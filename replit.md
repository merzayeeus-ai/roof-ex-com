...
- **Static Site Generation**: All static pages + 25 Field Notes posts are pre-rendered to HTML with embedded JSON-LD schema for optimal SEO. Field Notes are fetched live from CompanyCam API during each build. GitHub Actions auto-rebuilds every 6 hours.
...
- **Comprehensive SEO**: Extensive sitemap, image sitemap, `llms.txt`, `llms-full.txt`, speakable schema, social meta tags, rich SEO keywords, HowTo schema on all 8 service pages, SearchAction on homepage WebSite schema, enhanced E-E-A-T Person schema for founder. GTM permanently removed. Google Business Profile URL in `sameAs` across all schemas.
- **Local SEO / "Near Me" Optimization (Google 2026 Strategy)**: Per-city geo coordinates and zip codes in `server/city-geo.ts` (63 cities). City page LocalBusiness schema uses city-specific `addressLocality`, lat/lng, zip code, and `alternateName` arrays (`[City] Roofing Company`, `Roofer Near Me [City]`, etc.) for maximum "near me" signal. `areaServed` includes both `City` entity (with `containedInPlace: State`) and individual `PostalAddress` entries for every ZIP code — tells Google exactly which zip codes match which city. City+service pages (315 pages) now include FAQPage schema with "near me" questions, `alternateName`, `areaServed` with zip codes, and `sameAs` links. City guide pages include LocalBusiness schema with geo data, `areaServed` zip codes, and "near me" FAQ questions. ZIP-to-neighborhood mapping in `client/src/data/zip-areas.ts` displays specific area names (e.g., "94110 — Mission District") on city and guide pages. Keywords across all city pages include: `roofer near me, roofers near me, roofing company near me, roof contractor near me, roofing contractor near me, roofer near me [City] CA, roofing company near me [City], [City] roofing contractor, [City] CA roofer [ZIP]`. Meta descriptions include ZIP codes and county for local relevance. FAQPage schema expanded to 9 questions per city including "Who is the best roofing company near [City]?", "Where can I find a roofer near me in [City]?", and "What ZIP codes does ROOF EXPRESS serve near [City]?".
...
- **Image Optimization**: Full WebP migration and preloading of LCP images. Static hero shells injected into all pre-rendered HTML pages during Cloudflare build — LCP image renders in raw HTML before React JS loads, eliminating JS-blocking LCP delay. Homepage uses `<picture>` with mobile/desktop sources; all other pages get a fixed-position hero with gradient overlay that fades out after DOMContentLoaded. B&A badges use `text-black` on `bg-amber-500` for WCAG AA contrast (not `text-white`).
...
- **City+Service Sub-Pages**: 315 pages at `/{city}/{service}` URLs (63 cities × 5 services: roof-repair, roof-replacement, residential-roofing, commercial-roofing, gutters). Each page has unique city+service content with question-based headings, neighborhood-specific details, city+service FAQ, LocalBusiness JSON-LD with city geo coordinates, BreadcrumbList schema, and cross-links to other services in the same city. Data in `client/src/data/city-service-content.ts`, component in `client/src/pages/city-service-page.tsx`. Total sitemap: 571 URLs.
- **Multi-Platform Review Showcase**: `ReviewShowcase` component (`client/src/components/review-showcase.tsx`) displays ratings from 6 platforms (Google, Diamond Certified, Yelp, BBB, Angi, Nextdoor) in a horizontal badge bar. Integrated on homepage, all 8 service pages, and all 63 city pages.
- **Internal Linking Strategy**: Field Notes cross-links on all 7 service pages, city-specific field note links on all 63 city pages and city guide pages, related article links on service pages, city pages link to all 5 city+service sub-pages, city+service pages cross-link to other services in same city, "Resources" mega menu in nav (Field Notes, City Guides, Blog, FAQ, Financing, Methodology), and 5-column footer (Services, Company, Resources, Popular Articles, Contact/Service Areas).
- **Title Dedup System**: `generatePostTitle()` in rebuild script tracks both raw and truncated page titles to prevent duplicate `<title>` tags across Field Notes. Falls back to second line, second sentence, then appends post ID suffix.
- **Auto Paragraph Splitting**: `splitLongParagraph()` breaks paragraphs longer than 3 sentences into shorter chunks for better Semrush readability scores and Google content quality signals.
...
- **City Hall Photos**: All 63 city hall/landmark photos stored locally as WebP in `client/src/assets/images/city-halls/`.
