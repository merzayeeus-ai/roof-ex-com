...
var PUBLIC_SERVICE_TAGS = new Set(Object.values(SERVICE_TAG_ALIASES));
var PUBLIC_SERVICE_TAG_IDS = /* @__PURE__ */ new Map([
  ["Roof Repair", "roof-repair"],
  ["Roof Replacement", "roof-replacement"],
  ["Asphalt Shingle Roofing", "asphalt-shingle-roofing"],
  ["Commercial Systems", "commercial-systems"],
...
var CITY_BY_NORMALIZED_NAME = /* @__PURE__ */ new Map();
...
function parseTaggedJobPhotos(payload) {
  ...
  return payload.map((value, index) => {
    ...
    ids.add(id);
    const thumbnail = localAssetPath(value.thumbnail, "thumbnail", index);
    const fullSize = localAssetPath(value.fullSize, "fullSize", index);
...
function getPublicGalleryTags(photos) {
  const tags = /* @__PURE__ */ new Set();
  for (const photo of photos) {
    for (const tag of photo.tags) {
      const normalized = normalizeGalleryTag(tag);
      if (normalized && PUBLIC_SERVICE_TAGS.has(normalized)) tags.add(normalized);
    }
  }
  return [...tags].map((name) => ({
    id: PUBLIC_SERVICE_TAG_IDS.get(name) ?? normalizeGalleryTag(name).toLowerCase().replace(/[^a-z0-9]+/g, "-"),
    name
  }));
}
...
async function readFieldNotesAsset(request, assets, summaryOnly = false) {
  ...
  for (const post of archive.posts) {
    if (typeof post.id !== "string" || typeof post.slug !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(post.slug) || seen.has(post.slug) || typeof post.title !== "string" || typeof post.localImage !== "string" || !/^\/images\/field-notes\/[\w-]+\.webp$/.test(post.localImage) || !summaryOnly && typeof post.description !== "string") {
...
async function handleCompanyCamPhotos(request, assets) {
  ...
  if (["tag", "city"].some((key) => search.getAll(key).length > 1 || (search.get(key)?.length || 0) > 100)) {
    return jsonResponse({ error: "Invalid gallery filter" }, 400, { "Cache-Control": "no-store" });
  }
  const requestedTag2 = search.get("tag");
  if (requestedTag2 && requestedTag2.trim().toLowerCase() === "wiki") {
    try {
  ...
  try {
    const photos = filterTaggedJobPhotos(await readTaggedGallery(request, assets), {
      tag: requestedTag2 || void 0,
      city: search.get("city") || void 0
    });
    return jsonResponse({ photos, source: "tagged-local-sync" }, 200, {
...
async function readTaggedGallery(request, assets) {
  if (!assets || typeof assets.fetch !== "function") throw new Error("Asset binding unavailable");
  const response = await assets.fetch(new Request(new URL("/data/job-photos.json", request.url), { method: "GET" }));
  if (!response.ok) throw new Error("Job photo manifest unavailable");
  return parseTaggedJobPhotos(await response.json());
}
async function handleCompanyCamTags(request, assets) {
  try {
    return jsonResponse(
      { tags: getPublicGalleryTags(await readTaggedGallery(request, assets)) },
      200,
      { "Cache-Control": "public, max-age=60, must-revalidate" }
    );
  } catch {
    return jsonResponse(
      { error: "Job photo manifest unavailable" },
      503,
...
function isAdminPath(pathname) {
  return pathname === "/admin" || pathname === "/admin/" || pathname === "/admin.html" || pathname.startsWith("/admin/");
}
async function fetchAssets(request, assets) {
  ...
  return changed ? new Response(response.body, {
...
var worker = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/companycam/tags") return handleCompanyCamTags(request, bindings.ASSETS);
    ...
    if (url.pathname.startsWith("/blog/field-notes/")) {
      try {
        ...
        let post = STATIC_FIELD_NOTES?.find((item) => item.id === identifier || item.slug === identifier);
        if (!post) {
          const posts = (await readFieldNotesAsset(request, bindings.ASSETS, true)).posts;
          post = posts.find((item) => item.id === identifier || item.slug === identifier);
        }
        if (!post) {
