        ...
        {viewMode === "showcase" ? (
          <div data-testid="city-gallery-showcase" className="max-w-3xl mx-auto">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-4 py-3 mb-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className="text-brandNavy text-sm font-black">{activeIdx + 1}<span className="text-slate-500 text-xs font-bold"> / {photos.length}</span></span>
                <button
...
export default function CityPage() {
  ...
  const zipData = citySlug ? cityZips[citySlug] : undefined;
  const zipStr = zipData ? zipData.zips.join(", ") : "";
  const neighborhoodNames = cityData ? cityData.neighborhoods.map(n => n.name).join(", ") : "";

  useSEO(
    cityData ? (cityData.seoTitle || `${cityData.name}, CA Roofing — Repair & Replacement | ROOF EXPRESS`) : "",
    cityData ? `Best roofer near ${cityData.name}, CA ${zipData?.zip || ""}. 5.0★ Google rated roofing company for roof repair, replacement, flat roofing & gutters. Serving ${cityData.name} ZIP codes ${zipData ? zipData.zips.slice(0, 3).join(", ") : ""}. ${zipData?.county || ""} County. Diamond Certified, GAF Master Elite. Free estimates — (650) 666-5554.` : undefined,
    cityData ? `roofer near me, roofing company near me, roofers near me, roofing contractor near me, roof repair near me, roof replacement near me, best roofer near ${cityData.name}, ${cityData.name} roofer, ${cityData.name} roofing company, ${cityData.name} roofing contractor, roof repair ${cityData.name} CA, roof replacement ${cityData.name}, roofing company near me ${cityData.name}, roofer near me ${cityData.name} CA, roof replacement company near me, emergency roofer near me, roofer ${zipData?.zip || ""}, ${zipData?.county || ""} County roofer, gutter installation near me, flat roof repair near me, skylight repair near me, roof contractor near me, ${cityData.name} CA roofer ${zipData?.zip || ""}` : undefined
  );
  ...
  return (
   ...
   <h2 className="text-3xl md:text-4xl font-black text-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div className="text-center">
              <p className="text-4xl md:text-5xl font-black text-brandOrangeLight mb-2">5.0★</p>
   ...
   <h2 className="text-4xl md:text-5xl font-black text-brandNavy mb-4">
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {[
              { q: `Who is the best roofing company near ${cityData.name}?`, a: `ROOF EXPRESS is the top-rated roofing company near ${cityData.name}, CA${zipData ? ` (${zipData.zips.slice(0, 3).join(", ")})` : ""}. We are Diamond Certified with a 5.0 Google rating, GAF Master Elite, Owens Corning Platinum Preferred, and CertainTeed Select ShingleMaster. We've completed thousands of residential and commercial roofing projects across ${cityData.name} and ${zipData?.county || "the Bay Area"} County.` },
              ...
              { q: `Where can I find a roofer near me in ${cityData.name}?`, a: `ROOF EXPRESS serves all of ${cityData.name}${zipData ? `, including ZIP codes ${zipData.zips.join(", ")}` : ""}. We cover every neighborhood: ${cityData.neighborhoods.map(n => n.name).join(", ")}. Call (650) 666-5554 for a free same-day estimate or visit roof-ex.com to book online.` },
              ...
              { q: `Do I need a permit to replace my roof in ${cityData.name}?`, a: `Yes, most ${cityData.name} roof replacements require a building permit from the ${zipData?.county || cityData.name} County Building Department. ROOF EXPRESS handles the entire permitting process for you, including the application, required inspections, and final sign-off. This is included in our service at no extra charge.` },
              ...
              { q: `Does ROOF EXPRESS offer emergency roof repair near ${cityData.name}?`, a: `Yes! We offer 24/7 emergency roof repair services throughout ${cityData.name}${zipData ? ` and all ${zipData.county} County` : ""}. If you have an active leak or storm damage, call us at (650) 666-5554 for immediate assistance. We can typically have a crew on-site the same day.` },
              { q: `Is ROOF EXPRESS licensed and insured in ${cityData.name}?`, a: `Absolutely. ROOF EXPRESS holds California State License Board (CSLB) License #1072766 with full workers' compensation and general liability insurance. We are also GAF Master Elite certified, Diamond Certified, and an Owens Corning Platinum Preferred contractor — serving all of ${zipData?.county || ""} County.` },
              { q: `What ZIP codes does ROOF EXPRESS serve near ${cityData.name}?`, a: `We provide full roofing services across all ${cityData.name} ZIP codes${zipData ? `: ${zipData.zips.join(", ")}` : ""}. Our service area covers ${cityData.neighborhoods.map(n => n.name).join(", ")} and surrounding areas in ${zipData?.county || "the Bay Area"} County, CA.` },
            ].map((faq, idx) => (
   ...
   <h2 className="text-4xl md:text-5xl font-black text-brandNavy mb-4">
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-6 max-w-screen-xl">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black text-brandNavy mb-3">Our Services in {cityData.name}</h2>
            <p className="text-slate-500 text-sm">Expert roofing services tailored for {cityData.name} homes and businesses</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 max-w-4xl mx-auto">
            {[
              { slug: "roof-repair", name: "Roof Repair", icon: "fa-tools" },
              { slug: "roof-replacement", name: "Roof Replacement", icon: "fa-home" },
              { slug: "residential-roofing", name: "Residential", icon: "fa-house-user" },
              { slug: "commercial-roofing", name: "Commercial", icon: "fa-building" },
              { slug: "gutters", name: "Gutters", icon: "fa-tint" },
            ].map((svc) => (
              <Link
                key={svc.slug}
      ...
      {zipData && (
        <ZipCodeSection cityName={cityData.name} county={zipData.county} zips={zipData.zips} testId="section-city-coverage-bottom" />
      )}
