...
export default function CityGuidePage() {
  ...
  const guideZipData = guideData?.slug ? cityZips[guideData.slug] : undefined;

  useSEO(
    guideData ? `${guideData.guideTitle} | ROOF EXPRESS` : "",
    guideData ? `Complete ${guideData.name} roofing guide 2026. Best roofer near ${guideData.name}, CA ${guideZipData?.zip || ""}. Permits, costs, materials & local problems. Serving ${guideData.name} ZIP codes ${guideZipData?.zips.slice(0, 3).join(", ") || ""}. ${guideZipData?.county || ""} County.` : undefined,
    guideData ? `${guideData.name} roofing guide 2026, roofer near me, roofing company near me, roofers near me, best roofer near ${guideData.name}, roof cost ${guideData.name} 2026, roofing permit ${guideData.name}, roof repair near me, roof replacement near me, ${guideData.name} roofer, ${guideData.name} roofing contractor, roofer near me ${guideData.name} CA, roofer ${guideZipData?.zip || ""}, ${guideZipData?.county || ""} County roofer, roof maintenance ${guideData.name}, best roofing material ${guideData.name}, roof contractor near me, roofing company near me ${guideData.name}` : undefined
  );
  ...
  return (
    <Layout>
      ...
      {guideZipData && (
        <ZipCodeSection cityName={guideData.name} county={guideZipData.county} zips={guideZipData.zips} testId="section-guide-coverage-bottom" />
      )}
